...
export default class Sheet {
	...
	*/
	range(start, end) {
		...
		if (startY >= this.data.length || startX >= this.data[startY].length || endY >= this.data.length || endX >= this.data[endY].length) {
			return [];
		}
	...
	*/
	...
	/**
	* Read a set of rows from the sheet.
	* @param {string} start The row to start the read from.
	* @param {number} max The maximum amount of rows to read (or -1 to read all).
	* @type {String}
	*/
	rows(start = 1, max = -1) {
		const [x, y] = this.__coordinates(`A${start}`);

		if (y >= this.data.length) {
			return [];
		}

		return this.data.filter((_, index) => ((index >= y) && (max < 0 || index <= max)) );
	}
